...
import { noAccess, searchUser, userNoAccess, searchGroup, getGroupWithDataAndMetadataAccess, getUserWithDataAndMetadataAccess, getAllUsersWithDataAndMetadataAccess, groupNoAccess } from '../fixtures/index.js';
...
When('the user clicks Give access button to give target user {string} data access and {string} metadata access', (dataaccess, metadataaccess) => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getUserWithDataAndMe;
    const expected = {
      object: (_getUserWithDataAndMe = getUserWithDataAndMetadataAccess(metadataaccess, dataaccess)) === null || _getUserWithDataAndMe === void 0 ? void 0 : _getUserWithDataAndMe.object
    };
...
When('the user clicks Give access button to give target group {string} data access and {string} metadata access', (dataaccess, metadataaccess) => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getGroupWithDataAndM;
    const expected = {
      object: (_getGroupWithDataAndM = getGroupWithDataAndMetadataAccess(metadataaccess, dataaccess)) === null || _getGroupWithDataAndM === void 0 ? void 0 : _getGroupWithDataAndM.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
...
Given('a sharing dialog with user item with {string} data access and {string} metadata access', (datalevel, metadatalevel) => {
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
...
Given('a sharing dialog with group item with {string} data access and {string} metadata access', (datalevel, metadatalevel) => {
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
...
Given('a sharing dialog with all users item with {string} data access and {string} metadata access', (datalevel, metadatalevel) => {
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
    body: getAllUsersWithDataAndMetadataAccess(metadatalevel, datalevel)
  });
  cy.visitStory('sharing-dialog', 'data');
  cy.contains('Sharing and access').should('be.visible');
...
When('the user sets the user data access level to {string}, and leaves metadata as {string}', (dataAccess, metadataAccess) => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getUserWithDataAndMe2;
    const expected = {
      object: (_getUserWithDataAndMe2 = getUserWithDataAndMetadataAccess(metadataAccess, dataAccess)) === null || _getUserWithDataAndMe2 === void 0 ? void 0 : _getUserWithDataAndMe2.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
...
When('the user sets the group data access level to {string}, and leaves metadata as {string}', (dataAccess, metadataAccess) => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getGroupWithDataAndM2;
    const expected = {
      object: (_getGroupWithDataAndM2 = getGroupWithDataAndMetadataAccess(metadataAccess, dataAccess)) === null || _getGroupWithDataAndM2 === void 0 ? void 0 : _getGroupWithDataAndM2.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
...
When('the user sets the all users data access level to {string}, and leaves metadata as {string}', (dataAccess, metadataAccess) => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getAllUsersWithDataA;
    const expected = {
      object: (_getAllUsersWithDataA = getAllUsersWithDataAndMetadataAccess(metadataAccess, dataAccess)) === null || _getAllUsersWithDataA === void 0 ? void 0 : _getAllUsersWithDataA.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
  ...
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
    body: getAllUsersWithDataAndMetadataAccess(metadataAccess, dataAccess)
  });
  cy.get('@all-users-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Data').click();
  });
...
Then('the user data access control should be set to {string}', newDataAccess => {
  cy.get('@user-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Data').within(() => {
...
Then('the group data access control should be set to {string}', newDataAccess => {
  cy.get('@group-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Data').within(() => {
      cy.contains(newDataAccess).should('be.visible');
    });
...
Then('the all users data access control should be set to {string}', newDataAccess => {
  cy.get('@all-users-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Data').within(() => {
      cy.contains(newDataAccess).should('be.visible');
    });
...
Then('the user metadata access control should remain {string}', metadataAccess => {
  cy.get('@user-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Metadata').within(() => {
...
Then('the group metadata access control should remain {string}', metadataAccess => {
  cy.get('@group-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Metadata').within(() => {
      cy.contains(metadataAccess).should('be.visible');
    });
...
Then('the all users metadata access control should remain {string}', metadataAccess => {
  cy.get('@all-users-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', 'Metadata').within(() => {
      cy.contains(metadataAccess).should('be.visible');
    });
...
When('a sharing dialog with user item with {string} for data and No access for metadata is visible', dataAccess => {
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
...
When('a sharing dialog with group item with {string} for data and No access for metadata is visible', dataAccess => {
  cy.intercept('GET', '/api/38/sharing?type=visualization&id=id', {
...
When('the user clicks to remove the access for the user from the {string} access select', type => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    const expected = {
...
When('the user clicks to remove the access for the group from the {string} access select', type => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    const expected = {
...
When('the user sets the user metadata access level to No access and leaves data access as {string}', dataAccess => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getUserWithDataAndMe3;
    const expected = {
      object: (_getUserWithDataAndMe3 = getUserWithDataAndMetadataAccess('No access', dataAccess)) === null || _getUserWithDataAndMe3 === void 0 ? void 0 : _getUserWithDataAndMe3.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
...
When('the user sets the group metadata access level to No access and leaves data access as {string}', dataAccess => {
  cy.intercept('PUT', '/api/38/sharing?type=visualization&id=id', req => {
    var _getGroupWithDataAndM3;
    const expected = {
      object: (_getGroupWithDataAndM3 = getGroupWithDataAndMetadataAccess('No access', dataAccess)) === null || _getGroupWithDataAndM3 === void 0 ? void 0 : _getGroupWithDataAndM3.object
    };
    expect(req.body).to.deep.equal(expected);
    req.reply({
      statusCode: 200
...
Then('the user {string} access level options do not contain Remove access', sharingType => {
  cy.get('@user-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', sharingType).click();
  });
  cy.contains('Remove access').should('not.exist');
});
Then('the group {string} access level options do not contain Remove access', sharingType => {
  cy.get('@group-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', sharingType).click();
  });
  cy.contains('Remove access').should('not.exist');
});
Then('the all users {string} access level options do not contain Remove access', sharingType => {
  cy.get('@all-users-list-item').within(() => {
    cy.contains('[data-test="dhis2-uicore-singleselect"]', sharingType).click();
  });
  cy.contains('Remove access').should('not.exist');
