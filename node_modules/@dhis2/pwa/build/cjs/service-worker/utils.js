...
const APP_ADAPTER_URL_PATTERNS = [/\/api(\/\d+)?\/system\/info/,
// from ServerVersionProvider
/\/api(\/\d+)?\/userSettings/,
// useLocale
...
const APP_SHELL_URL_FILTER_PATTERNS = JSON.parse(process.env.REACT_APP_DHIS2_APP_PWA_CACHING_PATTERNS_TO_OMIT_FROM_APP_SHELL ||
// A deprecated fallback option:
process.env.REACT_APP_DHIS2_APP_PWA_CACHING_PATTERNS_TO_OMIT || '[]').map(pattern => new RegExp(pattern));
const OMIT_EXTERNAL_REQUESTS_FROM_APP_SHELL = process.env.REACT_APP_DHIS2_APP_PWA_CACHING_OMIT_EXTERNAL_REQUESTS_FROM_APP_SHELL === 'true' ||
// Deprecated option:
...
function setUpKillSwitchServiceWorker() {
  ...
  self.addEventListener('activate', async () => {
    ...
    // Delete DB
    await (0, _sectionsDb.deleteSectionsDB)();
